...
- **Serving Unit Model (July 2026)**: Structured serving fields (`serving_value`, `serving_unit`, `serving_unit_kind`) now coexist with legacy `oz`/`g` fields on Nutrition, Food, and Recipe records. The structured fields are the preferred path going forward; legacy fields remain for compatibility during the rollout window. All new code should read structured fields first and fall back to legacy only when structured fields are null. See `plans/plan-issue14.md` for the full feature plan and slice breakdown.
- **Multi-Serving Model (July 2026)**: The `nutrition_alternative` table supports multiple serving views per Food. Each alternative is a junction record pairing a Food with its own dedicated Nutrition profile (e.g., "per slice" nutrition for a pizza where the primary nutrition is for 1/4 of the pizza). The primary `nutrition` record on Food remains the *default* view used by Recipe/DailyLog aggregation. Alternatives are display-only — Recipes and DailyLogs always consume the primary Nutrition. The `serving_value` / `serving_unit` / `serving_unit_kind` fields on Nutrition now represent the *default* serving view; alternatives carry their own serving metadata. For lifecycle details: `NutritionAlternative.add()` creates a new Nutrition record + junction; `NutritionAlternative.delete()` cascades to the Nutrition; `Food.update()` replaces all alternatives; `Food.delete()` cascades to all alternatives and their Nutritions. See `plans/plan-issue14b.md` for the full feature plan and slice breakdown.
